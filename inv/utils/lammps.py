...
def rotate2lammps(atoms):
    ...
    if np.dot(np.cross(A, B), C) > 0:
        ...
        return Atoms(
            positions=newpos, numbers=atoms.numbers, cell=cell.T, pbc=atoms.pbc.tolist()
        )
...
def atoms2lammpscmds(atoms, typeMap, create_atoms_cmdstring=None, mass_cmdstring=None):
    ...
    create_commands = genatomcmdlist(
        _atoms, typeMap, create_atoms_cmdstring, mass_cmdstring
    )
