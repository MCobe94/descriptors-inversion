...
class Dataset():  
    ...
    def fromFiles(cls, *args, types):
        ...
        *args : str
        ...
        for arg in args:
            ...
            data['structures'] += list(map(Atoms, read(arg,
                                                       index=':',
                                                       format='extxyz'
                                                       )
                                           )
        ...
        for i in range(len(data['structures'])):                   
            ...
            if np.any(data['structures'][i].pbc) == False:
                ...
                data['structures'][i].wrap(pbc=True, center=True, pretty_translation=True)          
    ...
    def __getitem__(self, index):
        ...
        return Dataset({'all_types':self.data['all_types'], 
                        'files_info':None,
                        'descriptors':self.data['descriptors'][index] if len(self.data['descriptors'])>0 else [],
                        'descriptors_d':self.data['descriptors_d'][index] if len(self.data['descriptors_d'])>0 else [],
                        'index':self.data['index'][index] if len(self.data['index'])>0 else [],           
                        'structures':self.data['structures'][index] if len(self.data['structures'])>0 else [],
                        'types':self.data['types'][index] if len(self.data['types'])>0 else [],
                        })
    ...
    def computeBispectrum(self, rcutfac, rfac0, twojmax):
        ...
        for i in range(len(self)):
            b, b_d = Bispectrum(self.data['structures'][i],
                                [Atom(_).number for _ in sorted(set([Atom(t).symbol for t in self.data['types'][i]]))],       
                                rcutfac,
                                rfac0,
                                twojmax
                                )
    ...
    def computeBispectrumDask(self, rcutfac, rfac0, twojmax, n_workers=4): 
        ...
        for i in range(len(self)):
            b = delayed(Bispectrum)(self.data['structures'][i],
                                    [Atom(_).number for _ in sorted(set([Atom(t).symbol for t in self.data['types'][i]]))],  
                                    rcutfac,
                                    rfac0,
                                    twojmax
    ...
    def computeInversion(self, targets, rcutfac, rfac0, twojmax, N=100, gamma=0.0000008, eta=5e-2, nu=1e-4):
        '''
        ...
        for i in range(len(targets)):
            inverted, loss = Inversion(self[0],
                                       targets[i],
                                       rcutfac,
                                       rfac0,
                                       twojmax,
    ...
    def computeInversionDask(self, targets, rcutfac, rfac0, twojmax, N=100, gamma=0.0000008, eta=5e-2, nu=1e-4, n_workers=4):
        '''
        ...
        for i in range(len(targets)):
            inversion = delayed(Inversion)(self[0],
                                           targets[i],
                                           rcutfac,
                                           rfac0,
                                           twojmax,
