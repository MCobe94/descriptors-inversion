...
def Inversion(start, target, rcutfac, rfac0, twojmax, N, gamma, eta, nu):
    ...
    target_b, target_bd = Bispectrum(
        target.data["structures"],
        [
            Atom(_).number
            for _ in sorted(set([Atom(t).symbol for t in target.data["types"]]))
        ],
        rcutfac,
        rfac0,
        twojmax,
    ...
    for step in range(N):
        ...
        prototype_b, prototype_bd = Bispectrum(
            start.data["structures"],
            [
                Atom(_).number
                for _ in sorted(set([Atom(t).symbol for t in start.data["types"]]))
            ],
            rcutfac,
            rfac0,
            twojmax,
        ...
        for i in range(n_atoms):
            sum_b[start.data["all_types"].index(start.data["types"][i])] += prototype_b[
                i
            ]
        ...
        for i in range(n_atoms):
            for j in range(len(start.data["all_types"])):
                for k in range(3):
                    grad_loss[i][k] += -2 * np.dot(
                        sum_bt[j] - sum_b[j], prototype_bd[i][j][k]
                    )

            start.data["structures"].positions[i] = (
                np.array(start.data["structures"].positions[i])
                + gamma * np.array(grad_loss[i])
                + eta * np.exp(-nu * step) * np.random.uniform()
            )
