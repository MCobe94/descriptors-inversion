...
if __name__ == "__main__":
    ...
    start = Dataset.fromFiles(path + name + "_start.xyz", types=types)

    target = Dataset.fromFiles(path + "deformed.xyz", types=types)

    # Test Bispectrum inversion

    inverted, loss = start.computeInversionDask(
        target[0:N_config],
        rcutfac=3,
        rfac0=0.99,
        twojmax=8,
    ...
    for n in range(len(inverted)):
        ...
        if n == 0:
            ...
            write(
                "out_data/" + name + ".xyz", inverted[n], format="extxyz", append=True
            )
